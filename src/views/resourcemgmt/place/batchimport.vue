<template>
  <div class="content">
    <div style="font-weight: bold; margin: 15px 0px 30px 35px">导入场地</div>
    <el-form :model="place" ref="place" :rules="rules" label-width="100px" class="" :label-position="labelPosition">
      <el-form-item prop="schoolYear" :rules="{required: true, message: '请选择学年', trigger: 'blur'}" label="学年：">
          <el-select v-model="place.schoolYear" placeholder="请选择" size="mini" :disabled="disabledFlag" @change="schoolYearChange">
            <el-option
      ...
      <el-form-item label="提示：" v-model="place.tips">
          <span class="tips">第一步，下载场地信息模板，用下载的模板填写场地基本信息；</span> <a @click="downloadTemplate"
      ...
      <div style="margin-left: 410px">
        <div style="position: relative; width: 280px;" v-show="showProgress">
          ...
          <div style="right: 40px; bottom: 14px; padding-right: 12px;" class="progress-inner-text"
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      // 拼接上传地址
      this.url = this.baseApi + '/api/auth/v1/field/saveSchoolFieldFromExcel'
      // 初始化上传状态图标
    ...
    methods: {
      ...
      // 下载模版
      downloadTemplate() {
        this.$store.dispatch('PlaceTemplate').then(res => {
        ...
        setTimeout(function() {
          ...
          if ((_this.successNum + _this.failNum) === _this.fileSize) {
            if (_this.failNum === 0) {
              ...
              // 上传成功，跳转列表
              setTimeout(function() {
                _this.$router.push('/fieldManagement')
              }, 1000)
            } else {
